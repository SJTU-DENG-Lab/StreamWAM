"""Token-action ActionDiT builders for StreamWAM."""
...
def load_action_dit_init(
    ...
    if not init_from:
        return
    from streamwam.checkpointing import load_action_dit_backbone_init

    info = load_action_dit_backbone_init(action_dit, init_from, head_init=head_init)
    print(
        f"[StreamWAM] Loaded ActionDiT init from {init_from} "
