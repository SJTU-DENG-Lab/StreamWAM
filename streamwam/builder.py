...
from streamwam.data.lerobot import (
...
from streamwam.taxonomy import (
...
LOGGER = logging.getLogger("streamwam.builder")


def build_framework(config: Any, device: str = "cpu", dtype=None):
    """Build a StreamWAM taxonomy model."""
    ...
    if model_family == MOT_WAM:
        if config.framework.type != "mot":
            raise ValueError("StreamWAM mot_wam currently requires framework.type='mot'")
        ...
        if action_representation != TOKEN_ACTION:
            raise NotImplementedError(
                "StreamWAM mot_wam currently trains only taxonomy.action_representation='token_action'."
            )
        from streamwam.backbone import build_backbone
        from streamwam.wam import MoTWAM, ACStreamWAM
        ...
        if variant == "ac-stream":
            LOGGER.info("Building StreamWAM AC-Stream MoT WAM with backbone=%s", config.backbone.type)
            return ACStreamWAM(backbone, config.framework, device=device, dtype=dtype)
        if variant != "standard":
            raise ValueError(
                f"Unsupported mot_wam framework.variant={variant!r}; "
                "expected 'standard' or 'ac-stream'"
            )
        LOGGER.info("Building StreamWAM MoT WAM with backbone=%s", config.backbone.type)
    ...
    if model_family == FEATURE_CONDITIONED_ACTION_MODEL:
        if config.framework.type not in {"feature_conditioned", "feature_conditioned_action"}:
            raise ValueError(
                "StreamWAM feature_conditioned_action_model requires "
                "framework.type='feature_conditioned'"
            )
        from streamwam.backbone import build_backbone
        from streamwam.wam import FeatureConditionedActionModel

        backbone = build_backbone(config.backbone, device=device, dtype=dtype)
        LOGGER.info("Building StreamWAM feature-conditioned action model with backbone=%s", config.backbone.type)
    ...
    if model_family == SHARED_DIT_WAM:
        if config.framework.type not in {"shared_dit", "shared_dit_wam"}:
            raise ValueError("StreamWAM shared_dit_wam requires framework.type='shared_dit'")
        action_representation = getattr(getattr(config, "taxonomy", None), "action_representation", TOKEN_ACTION)
        if action_representation != TOKEN_ACTION:
            raise NotImplementedError(
                "StreamWAM shared_dit_wam currently trains only taxonomy.action_representation='token_action'."
            )
        from streamwam.backbone import build_backbone
        from streamwam.wam import SharedDiTWAM

        backbone = build_backbone(config.backbone, device=device, dtype=dtype, load_dit=False)
        LOGGER.info("Building StreamWAM shared-DiT WAM with backbone=%s", config.backbone.type)
        return SharedDiTWAM(backbone, config.framework, device=device, dtype=dtype)

    raise AssertionError(f"Unhandled StreamWAM model family: {model_family}")
...
def _load_or_compute_lerobot_stats(
    ...
    from streamwam.data.lerobot import compute_action_stats, compute_state_stats, load_lerobot_stats, save_lerobot_stats
...
def _ensure_text_caches(config: StreamWAMConfig, roots: list[str]) -> None:
    ...
    lock_path = cache_dir / ".streamwam_text_cache.lock"
    while True:
        try:
            fd = os.open(lock_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
            os.close(fd)
            try:
                ...
                from streamwam.backbone import build_text_encoder
                ...
                LOGGER.info("Generating %d missing StreamWAM text caches in %s", len(missing), cache_dir)
...
def build_dataset(config: StreamWAMConfig, is_training: bool = True, text_dim: int = 4096):
    """Build the StreamWAM dataset."""

    from torch.utils.data import ConcatDataset, Subset

    from streamwam.data.lerobot import LeRobotDataset, LeRobotSyntheticDataset, compute_action_stats, compute_state_stats
...
def build_trainer(model: Any, config: StreamWAMConfig):
    """Build the StreamWAM trainer."""
    ...
    return StreamWAMTrainer(model, train_dataset, val_dataset, config.training)
