...
from streamwam.backbone.base import BaseBackbone, BackboneInfo
from streamwam.config import BackboneConfig
from streamwam.checkpointing import infer_backbone_info
from streamwam.modules.wan_block import (
    DiTBlock,
...
class Wan22Backbone(BaseBackbone):
    ...
    def build_shared_dit_core(
        ...
        *,
        ...
        from streamwam.modules.causal_wan import CausalWanModel
    ...
    def _load_shared_dit_video_weights(self, shared_dit: nn.Module, dtype: torch.dtype) -> None:
        ...
        if not model_dir.exists():
            print(f"[StreamWAM] Shared-DiT video init skipped: checkpoint dir not found at {model_dir}")
            return
        ...
        if not index_file.exists() and not single_file.exists():
            print(f"[StreamWAM] Shared-DiT video init skipped: no safetensors under {model_dir}")
            return
        ...
        if stagger_sec > 0 and local_rank > 0:
            ...
            print(
                f"[StreamWAM][rank={rank}/local={local_rank}] Waiting {delay:.0f}s before shared-DiT checkpoint load "
        ...
        seen: set[str] = set()
        print(f"[StreamWAM][rank={rank}/local={local_rank}] Loading shared-DiT video init from {model_dir}", flush=True)

        def load_compatible_state(path: Path, shard_idx: int, num_shards: int) -> None:
            start = time.time()
            print(
                f"[StreamWAM][rank={rank}/local={local_rank}] loading shard {shard_idx}/{num_shards}: {path.name}",
                flush=True,
            )
            ...
            gc.collect()
            print(
                f"[StreamWAM][rank={rank}/local={local_rank}] loaded shard {shard_idx}/{num_shards} "
        ...
        print(
            "[StreamWAM] Initialized shared-DiT directly from video DiT checkpoint "
