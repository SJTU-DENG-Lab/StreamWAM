...
Reuses Wan2.2-compatible :class:`streamwam.modules.wan_block.DiTBlock`, so its
``blocks.*`` state_dict layout matches the video DiT exactly. This is what
allows ``python -m streamwam.tools.preprocess_action_dit_init`` to copy or interpolate
