"""Checkpoint-specific modules for the FastWAM AC-Stream MoT variant."""
...
def build_ac_stream_policy_mask(
    *,
    video_seq_len: int,
    action_seq_len: int,
    video_tokens_per_frame: int,
    ...
    if int(action_seq_len) != AC_STREAM_ACTION_HORIZON:
        raise ValueError(
            f"AC-Stream policy stream requires {AC_STREAM_ACTION_HORIZON} actions, "
            f"got {action_seq_len}"
        )
    if int(known_prefix_length) not in (0, AC_STREAM_DELAY):
        raise ValueError(f"AC-Stream prefix length must be 0 or {AC_STREAM_DELAY}")
...
def build_ac_stream_condition_mask(
    *,
    video_seq_len: int,
    ...
    if int(condition_seq_len) != AC_STREAM_CONDITION_SLOTS:
        raise ValueError(
            f"AC-Stream condition stream requires {AC_STREAM_CONDITION_SLOTS} slots, "
    ...
    if video_seq_len != 3 * tokens_per_frame:
        raise ValueError(
            "AC-Stream video tokens must contain exactly z0/z1/z2; got "
            f"video_seq_len={video_seq_len}, tokens_per_frame={tokens_per_frame}"
        )
    condition_start = video_seq_len
    prefix_end = condition_start + AC_STREAM_DELAY
...
def build_starwam_rtc_policy_mask(
    *,
    batch_size: int,
    video_seq_len: int,
    policy_seq_len: int,
    ...
    """Build yzy StarWAM RTC policy-query routing over ``[video, policy]``."""

    if int(policy_seq_len) != AC_STREAM_ACTION_HORIZON:
        raise ValueError(
            f"StarWAM RTC policy stream requires {AC_STREAM_ACTION_HORIZON} actions, "
            f"got {policy_seq_len}"
        )
    if int(known_prefix_length) not in (0, AC_STREAM_DELAY):
        raise ValueError(f"StarWAM RTC prefix length must be 0 or {AC_STREAM_DELAY}")
    batch = int(batch_size)
    policy_len = int(policy_seq_len)
    video_len = int(video_seq_len)
    first_frame = int(video_tokens_per_frame)
    delay = torch.full(
        (batch, 1, 1),
        int(known_prefix_length),
        device=device,
        dtype=torch.long,
    )
    query_position = torch.arange(policy_len, device=device).view(1, -1, 1)
    key_position = torch.arange(policy_len, device=device).view(1, 1, -1)
    d0 = delay == 0
    clean_query = query_position < delay
    clean_key = key_position < delay

    video = torch.zeros((batch, policy_len, video_len), dtype=torch.bool, device=device)
    video[:, :, :first_frame] = True
    video[:, :, first_frame:] = (delay == AC_STREAM_DELAY) & (~clean_query)
    policy = d0 | (~clean_query) | clean_key
    return torch.cat((video, policy), dim=-1).unsqueeze(1)


def build_starwam_rtc_condition_mask(
    *,
    batch_size: int,
    condition_seq_len: int,
    video_tokens_per_frame: int,
    ...
    """Build yzy StarWAM RTC condition-query routing over ``[z0, condition]``."""

    if int(condition_seq_len) != AC_STREAM_CONDITION_SLOTS:
        raise ValueError(
            f"StarWAM RTC requires {AC_STREAM_CONDITION_SLOTS} condition slots, "
            f"got {condition_seq_len}"
        )
    if int(known_prefix_length) not in (0, AC_STREAM_DELAY):
        raise ValueError(f"StarWAM RTC prefix length must be 0 or {AC_STREAM_DELAY}")
    batch = int(batch_size)
    condition_len = int(condition_seq_len)
    delay = torch.full(
        (batch, 1, 1),
        int(known_prefix_length),
        device=device,
    ...
    query_position = torch.arange(condition_len, device=device).view(1, -1, 1)
    key_position = torch.arange(condition_len, device=device).view(1, 1, -1)
    known_query = query_position < delay
    known_key = key_position < delay
    condition = (~known_query) | known_key
    z0 = torch.ones(
        (batch, condition_len, int(video_tokens_per_frame)),
        dtype=torch.bool,
        device=device,
    )
    return torch.cat((z0, condition), dim=-1).unsqueeze(1)


class ACStreamSlotEncoder(nn.Module):
    ...
    def forward(self, action_tokens: Tensor, known_mask: Tensor) -> Tensor:
        if tuple(action_tokens.shape[1:]) != (
            ...
            raise ValueError(
                "AC-Stream action slots must have shape "
                f"[B,{AC_STREAM_CONDITION_SLOTS},{self.hidden_dim}], got "
                f"{tuple(action_tokens.shape)}"
            )
        if tuple(known_mask.shape) != tuple(action_tokens.shape[:2]):
            raise ValueError(
                f"AC-Stream known mask must be {tuple(action_tokens.shape[:2])}, "
...
class ACStreamMoT(MoT):
    ...
    def forward_ac_stream(
        ...
        *,
        ...
        if self.expert_order != ["video", "action"]:
            raise ValueError("AC-Stream requires video/action expert order")
        if set(expert_states) != {"video", "action", "condition"}:
            raise ValueError("AC-Stream requires video, action, and condition streams")
        ...
        if video_len != 3 * tokens_per_frame:
            raise ValueError("AC-Stream requires z0/z1/z2 video tokens")
        if action_len != AC_STREAM_ACTION_HORIZON:
            raise ValueError(f"AC-Stream requires {AC_STREAM_ACTION_HORIZON} policy actions")
        if condition_len != AC_STREAM_CONDITION_SLOTS:
            raise ValueError(f"AC-Stream requires {AC_STREAM_CONDITION_SLOTS} condition slots")
        if tuple(action_condition_active.shape) != (tokens["video"].shape[0],):
            raise ValueError("AC-Stream action_condition_active must be [B]")
    ...
    def forward_starwam_rtc(
        ...
        *,
        ...
        """Run the yzy StarWAM RTC three-stream attention routing."""

        if self.expert_order != ["video", "action"]:
            raise ValueError("StarWAM RTC requires video/action expert order")
        if set(expert_states) != {"video", "action", "condition"}:
            raise ValueError("StarWAM RTC requires video, action, and condition streams")
        video_expert = self.experts["video"]
        action_expert = self.experts["action"]
        tokens = {
            name: expert_states[name]["tokens"]
        ...
        tokens_per_frame = int(video_tokens_per_frame)
        if int(tokens["video"].shape[1]) != 3 * tokens_per_frame:
            raise ValueError("StarWAM RTC requires z0/z1/z2 video tokens")
        active = action_condition_active.to(
            device=tokens["video"].device,
            dtype=tokens["video"].dtype,
        ).view(-1, 1, 1)
        ...
        for layer_index in range(self.num_layers):
            video_block = video_expert.blocks[layer_index]
            action_block = action_expert.blocks[layer_index]
            qv, kv, vv = video_block.get_qkv(
                tokens["video"],
                expert_states["video"]["t_mod"],
                expert_states["video"]["freqs"],
            )
            qp, kp, vp = action_block.get_qkv(
                tokens["action"],
                expert_states["action"]["t_mod"],
                expert_states["action"]["freqs"],
            )
            qc, kc, vc = action_block.get_qkv(
                tokens["condition"],
                expert_states["condition"]["t_mod"],
                expert_states["condition"]["freqs"],
            )
            video_output = self._directed_attention(
                qv,
                kv,
            ...
            policy_output = self._directed_attention(
                qp,
                torch.cat((kv, kp), dim=1),
                torch.cat((vv, vp), dim=1),
                action_block,
                policy_attention_mask,
            )
            condition_output = self._directed_attention(
                qc,
                torch.cat((kv[:, :tokens_per_frame], kc), dim=1),
                torch.cat((vv[:, :tokens_per_frame], vc), dim=1),
                action_block,
            ...
            for name, block, attention_output in (
                ("video", video_block, video_output),
                ("action", action_block, policy_output),
                ("condition", action_block, condition_output),
            ):
                state = expert_states[name]
                tokens[name] = block.post_attention(
    ...
    def forward_ac_stream_accelerated(
        self,
        *,
        ...
        """Reference-shaped fixed-geometry AC-Stream core used by TorchInductor."""
    ...
    def forward_starwam_rtc_accelerated(
        self,
        *,
        tokens_all: dict[str, Tensor],
        freqs_all: dict[str, Tensor],
        ...
        """Functional yzy StarWAM RTC core used by TorchInductor."""
        ...
        for layer_index in range(self.num_layers):
            ...
            qv, kv, vv = projected["video"]
            qp, kp, vp = projected["action"]
            qc, kc, vc = projected["condition"]
            video_output = self._directed_attention(
                qv,
                kv,
                vv,
