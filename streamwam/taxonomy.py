"""Taxonomy helpers for StreamWAM model families."""
...
def normalize_model_family(model_family: str | None) -> str:
    ...
    try:
        ...
        raise ValueError(f"Unknown StreamWAM model_family={model_family!r}; allowed families: {allowed}") from e
...
def validate_taxonomy(config: Any) -> str:
    ...
    package = getattr(taxonomy, "package", "streamwam")
    if package != "streamwam":
        raise ValueError(f"taxonomy.package must be 'streamwam', got {package!r}")
