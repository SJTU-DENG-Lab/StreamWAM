"""Benchmark-agnostic StreamWAM policy wrapper for closed-loop evaluation.
...
of the LIBERO ``examples/libero/rollout.py`` script (which is left untouched):
    ...
    BenchmarkAdapter (per-benchmark)  ->  StreamWAMPolicy (this file)  ->  model
...
chunks back into environment actions. ``StreamWAMPolicy`` owns everything generic:
...
from streamwam.checkpointing import (
