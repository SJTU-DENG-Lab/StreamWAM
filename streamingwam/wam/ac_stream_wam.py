...
from streamingwam.modules.ac_stream import ACStreamMoT, ACStreamSlotEncoder
from streamingwam.modules.ac_stream import build_ac_stream_condition_mask, build_ac_stream_policy_mask
from streamingwam.modules.ac_stream import (
    build_starwam_rtc_condition_mask,
    build_starwam_rtc_policy_mask,
)
from streamingwam.training.flow import build_inference_schedule
...
class ACStreamingWAM(MoTWAM):
    ...
    def infer_action(
        ...
        **kwargs,
        ...
        if method != "ac-stream":
            raise ValueError(
                f"ACStreamingWAM requires sampling_method='ac-stream', got {method!r}"
            )
        architecture = getattr(
            self,
            "ac_stream_architecture",
