...
:class:`streamingwam.wam.mot_wam.MoTWAM.training_step`::
...
from streamingwam.config import DataConfig
...
def _parquet():
    try:
        ...
        raise ModuleNotFoundError(
            "LeRobot datasets require the Streaming-WAM training dependencies; "
    ...
    def _load_t5(self, task: str | None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not cache.is_file():
            raise FileNotFoundError(
                ...
                "Run `python -m streamingwam.tools.precompute_text_cache` or let training precompute it before workers start."
            )
