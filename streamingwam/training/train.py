"""Generic training entry point for Streaming-WAM recipes."""
...
logger = logging.getLogger("streamingwam.train")


def _configure_rank_logging():
    import os
    if int(os.environ.get("RANK", "0")) != 0:
        logging.getLogger("streamingwam").setLevel(logging.WARNING)
        logging.getLogger("streamingwam.train").setLevel(logging.WARNING)


def _maybe_prepare_streamingwam_text_cache(cfg: Any) -> None:
    ...
    if roots:
        from streamingwam.builder import _ensure_text_caches
        _ensure_text_caches(cfg, roots)


def main():
    _configure_rank_logging()
    parser = argparse.ArgumentParser(description="Streaming-WAM training")
    ...
    from streamingwam import build_framework, build_trainer
    from streamingwam.config import config_to_dict, load_config
    ...
    _maybe_prepare_streamingwam_text_cache(config)
