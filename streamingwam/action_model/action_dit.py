"""Token-action ActionDiT builders for Streaming-WAM."""
...
def load_action_dit_init(
    ...
    if not init_from:
        return
    from streamingwam.checkpointing import load_action_dit_backbone_init

    info = load_action_dit_backbone_init(action_dit, init_from, head_init=head_init)
    print(
        f"[Streaming-WAM] Loaded ActionDiT init from {init_from} "
