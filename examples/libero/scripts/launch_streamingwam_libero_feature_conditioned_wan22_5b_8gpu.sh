#!/bin/bash
# Streaming-WAM LIBERO feature-conditioned Wan2.2 training on a single node with 8 GPUs.
set -euo pipefail

REPO_DIR=${REPO_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}
PY=${PY:-python}

RECIPE=${RECIPE:-examples/libero/configs/recipes/streamingwam_libero_feature_conditioned_wan22_5b.yaml}
...
cat <<EOF
[launch] mode: Streaming-WAM LIBERO feature-conditioned Wan2.2 single-node 8-GPU
