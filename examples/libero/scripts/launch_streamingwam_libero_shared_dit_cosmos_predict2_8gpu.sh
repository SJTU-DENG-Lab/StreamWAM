#!/bin/bash
# Shared-DiT Streaming-WAM LIBERO training with Cosmos-Predict2 on a single node with 8 GPUs.
set -euo pipefail

REPO_DIR=${REPO_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}
PY=${PY:-python}

RECIPE=${RECIPE:-examples/libero/configs/recipes/streamingwam_libero_shared_dit_cosmos_predict2.yaml}
...
cat <<EOF
[launch] mode: Shared-DiT Streaming-WAM LIBERO Cosmos-Predict2 single-node 8-GPU
...
exec $PY -m accelerate.commands.launch \
  ...
  --module streamingwam.training.train \
  --config "$RECIPE" \
