#!/bin/bash
# Shared-DiT StreamWAM LIBERO training with Cosmos-Predict2 on a single node with 8 GPUs.
set -euo pipefail
...
RECIPE=${RECIPE:-examples/libero/configs/recipes/streamwam_libero_shared_dit_cosmos_predict2.yaml}
...
cat <<EOF
[launch] mode: Shared-DiT StreamWAM LIBERO Cosmos-Predict2 single-node 8-GPU
...
exec $PY -m accelerate.commands.launch \
  ...
  --module streamwam.training.train \
  --config "$RECIPE" \
