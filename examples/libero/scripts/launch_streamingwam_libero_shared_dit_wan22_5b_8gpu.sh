#!/bin/bash
# Shared-DiT Streaming-WAM LIBERO training on a single node with 8 GPUs.
set -euo pipefail

REPO_DIR=${REPO_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}
PY=${PY:-python}

RECIPE=${RECIPE:-examples/libero/configs/recipes/streamingwam_libero_shared_dit_wan22_5b.yaml}
...
cat <<EOF
[launch] mode: Shared-DiT Streaming-WAM LIBERO single-node 8-GPU
