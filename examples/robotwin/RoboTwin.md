...
RoboTwin 2.0 workflow for Streaming-WAM: setup, training, and rollout against the official RoboTwin harness. See root [README.md](../../README.md) and [../libero/LIBERO.md](../libero/LIBERO.md).

SAPIEN needs an NVIDIA Vulkan stack; Streaming-WAM inference needs Torch/CUDA. Pick one of two modes depending on your environment:

- **local** (`local_policy.py`, `policy_mode: local`): when the Vulkan stack and Torch/Streaming-WAM are available in the **same env** — inference runs in-process, simplest.
...
examples/robotwin/
  ...
  local_policy.py           # in-process adapter (SAPIEN + Torch/Streaming-WAM in one env)
  client_policy.py          # socket client adapter (SAPIEN-only env)
  policy_server.py          # Streaming-WAM inference server (Torch/Streaming-WAM env)
...
```bash
# local (single env):
ln -s /ABS/PATH/Streaming-WAM/examples/robotwin  RoboTwin/policy/streamingwam_policy
# client (split env):
ln -s /ABS/PATH/Streaming-WAM/examples/robotwin  RoboTwin/policy/streamingwam_client
```

## 2. Environment

### 2.1 Training / server (Torch/Streaming-WAM)

```bash
conda create -n streamingwam python=3.11 -y
...
Streaming-WAM client adapter itself needs only `numpy` + the standard library. On
...
| --- | --- | --- | --- |
| `streamingwam_robotwin_mot_wan22_5b.yaml` | `mot_wam` | Wan2.2-TI2V-5B | Preprocessed ActionDiT payload required. |
| `streamingwam_robotwin_shared_dit_wan22_5b.yaml` | `shared_dit_wam` | Wan2.2-TI2V-5B | Backbone initialization only; no ActionDiT payload. |
...
```bash
RECIPE=examples/robotwin/configs/recipes/streamingwam_robotwin_mot_wan22_5b.yaml \
...
export CONDA_ENV=streamingwam
export RECIPE=examples/robotwin/configs/recipes/streamingwam_robotwin_mot_wan22_5b.yaml
export TRAIN_OVERRIDES='data.dataset_dirs=["/path/to/robotwin2.0"] backbone.pretrained_model_id=/path/to/Wan2.2-TI2V-5B framework.action_expert_init_from=/path/to/preprocessed/streamingwam_action_dit_init_wan22.pt training.output_dir=/path/to/output/streamingwam_robotwin_mot_wan22_5b data.text_embedding_cache_dir=/path/to/output/streamingwam_robotwin_mot_wan22_5b/text_embedding_cache data.action_stats_path=/path/to/output/streamingwam_robotwin_mot_wan22_5b/action_stats.json data.state_stats_path=/path/to/output/streamingwam_robotwin_mot_wan22_5b/action_stats.json'
...
Step 1: start one inference server per GPU in the Torch/Streaming-WAM env. The recipe
...
```bash
REPO=/path/to/Streaming-WAM \
PY=/path/to/streamingwam-env/bin/python \
RECIPE=examples/robotwin/configs/recipes/streamingwam_robotwin_mot_wan22_5b.yaml \
CKPT=/path/to/output/streamingwam_robotwin_mot_wan22_5b/checkpoint-27500/pytorch_model \
BACKBONE=/path/to/Wan2.2-TI2V-5B \
ACTION_STATS=/path/to/output/streamingwam_robotwin_mot_wan22_5b/action_stats.json \
TEXTCACHE=/path/to/output/streamingwam_robotwin_mot_wan22_5b/eval_text_cache \
ACTION_INIT=/path/to/preprocessed/streamingwam_action_dit_init_wan22.pt \
NUM_INFERENCE_STEPS=4 ACTION_NUM_INFERENCE_STEPS=4 \
SERVER_BIND=0.0.0.0 SERVER_PORT_BASE=8765 NGPU=8 \
bash examples/robotwin/scripts/launch_streamingwam_robotwin_policy_server.sh
```
...
REPO=/path/to/Streaming-WAM \
PY=/path/to/streamingwam-env/bin/python \
RECIPE=examples/robotwin/configs/recipes/streamingwam_robotwin_shared_dit_wan22_5b.yaml \
CKPT=/path/to/output/streamingwam_robotwin_shared_dit_wan22_5b/checkpoint-45000/pytorch_model \
BACKBONE=/path/to/Wan2.2-TI2V-5B \
ACTION_STATS=/path/to/output/streamingwam_robotwin_shared_dit_wan22_5b/action_stats.json \
TEXTCACHE=/path/to/output/streamingwam_robotwin_shared_dit_wan22_5b/eval_text_cache \
...
task-settings in the RoboTwin env. Set `CKPT_TAG=streamingwam27500` for MoT or
`CKPT_TAG=sharedit45000` for Shared-DiT.

```bash
ln -s /path/to/Streaming-WAM/examples/robotwin RoboTwin/policy/streamingwam_client
cd RoboTwin
...
If SAPIEN and Torch/Streaming-WAM share one env, run the harness directly with
`policy_mode local` (no server needed):

```bash
ln -s /path/to/Streaming-WAM/examples/robotwin  RoboTwin/policy/streamingwam_policy
cd RoboTwin
python script/eval_policy.py \
  --config policy/streamingwam_policy/deploy_policy.yml \
  --overrides policy_name streamingwam_policy task_name adjust_bottle \
    task_config demo_clean instruction_type unseen ckpt_setting streamingwam27500 seed 0 \
    policy_mode local num_inference_steps 4 action_num_inference_steps 4 \
    checkpoint /path/to/output/streamingwam_robotwin_mot_wan22_5b/checkpoint-27500/pytorch_model \
    overrides "backbone.pretrained_model_id=/path/to/Wan2.2-TI2V-5B data.action_stats_path=/path/to/output/streamingwam_robotwin_mot_wan22_5b/action_stats.json data.state_stats_path=/path/to/output/streamingwam_robotwin_mot_wan22_5b/action_stats.json data.text_embedding_cache_dir=/path/to/output/streamingwam_robotwin_mot_wan22_5b/text_embedding_cache"
```
...
modelscope download --model panshaohua/starwam --local_dir /path/to/streamingwam_ckpts
# MoT: CKPT=/path/to/streamingwam_ckpts/starwam-robotwin/mot/starwam_wan225b_robotwin_mot.pt
# Shared-DiT: CKPT=/path/to/streamingwam_ckpts/starwam-robotwin/sharedit/starwam_wan225b_robotwin_sharedit.pt
# ACTION_STATS=/path/to/streamingwam_ckpts/starwam-robotwin/action_stats.json
```
...
CKPT=/path/to/Streaming-WAM/checkpoints/fastwam_release/robotwin_uncond_3cam_384.pt \
BACKBONE=/path/to/Wan2.2-TI2V-5B \
ACTION_STATS=/path/to/Streaming-WAM/checkpoints/fastwam_release/robotwin_uncond_3cam_384_dataset_stats.json \
...
- Streaming-WAM MoT missing ActionDiT init: run Section 6.1 and set
