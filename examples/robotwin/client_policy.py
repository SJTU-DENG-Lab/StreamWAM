...
class RemoteStreamWAMModel:
    ...
    def _start_episode(self) -> None:
        if self._episode_started_ns is None:
            self._episode_started_ns = time.perf_counter_ns()
            self._replan_index = 0

    def finish_episode(self, success: bool | None = None) -> None:
        if self._episode_started_ns is None:
            return
        elapsed = (time.perf_counter_ns() - self._episode_started_ns) / 1e9
    ...
    def step(self, task_env: Any, observation: Optional[Dict[str, Any]]) -> None:
        ...
        if self.inference_mode == "ac-stream":
            if observation is None:
                raise ValueError("AC-Stream requires an observation at every action step")
            snapshot = self._snapshot(task_env, observation)
        ...
        elif int(getattr(task_env, "take_action_cnt", 0)) >= int(getattr(task_env, "step_lim", 1 << 30)):
            self.finish_episode(success=False)
