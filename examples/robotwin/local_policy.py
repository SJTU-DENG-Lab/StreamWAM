...
:class:`streamingwam.eval.policy.StreamingWAMPolicy`. This file only handles the
RoboTwin-specific pieces:
  ...
  * 3-camera observation -> the exact 384x320 grid used at training time
    ...
    reuses ``streamingwam.data.lerobot._resize_frames`` so eval pixels match training
    pixels bit-for-bit.
...
# Make the Streaming-WAM package importable when launched from the RoboTwin repo.
...
class RoboTwinStreamingWAMModel:
    """Wraps :class:`StreamingWAMPolicy` with RoboTwin obs/action conventions."""

    def __init__(self, policy: StreamingWAMPolicy) -> None:
        self.policy = policy
...
def get_model(usr_args: Dict[str, Any]) -> RoboTwinStreamingWAMModel:
    ...
    checkpoint_format = _get(usr_args, "checkpoint_format", "streamingwam")
    ...
    return RoboTwinStreamingWAMModel(policy)


def eval(TASK_ENV: Any, model: RoboTwinStreamingWAMModel, observation: Optional[Dict[str, Any]]) -> None:
    model.step(TASK_ENV, observation)


def reset_model(model: RoboTwinStreamingWAMModel) -> None:
    model.reset()
