#!/bin/bash
# Launch StreamWAM RoboTwin policy inference servers (one per GPU) in the
# Torch/StreamWAM environment. Each server binds SERVER_PORT_BASE+gpu and serves
# action-chunk inference over a socket; the RoboTwin SAPIEN client connects to
# these ports (see launch_streamwam_robotwin_rollout.sh). RECIPE selects MoT
# or Shared-DiT; inference steps default to that recipe unless overridden.
#
# Use this when SAPIEN and Torch/StreamWAM cannot share one environment (the
...
REPO="${REPO:?set REPO=/path/to/StreamWAM}"
PY="${PY:?set PY=/path/to/streamwam-env/bin/python}"
...
CHECKPOINT_FORMAT="${CHECKPOINT_FORMAT:-streamwam}"
