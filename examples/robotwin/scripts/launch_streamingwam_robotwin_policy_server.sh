#!/bin/bash
# Launch Streaming-WAM RoboTwin policy inference servers (one per GPU) in the
# Torch/Streaming-WAM environment. Each server binds SERVER_PORT_BASE+gpu and serves
# action-chunk inference over a socket; the RoboTwin SAPIEN client connects to
# these ports (see launch_streamingwam_robotwin_rollout.sh). RECIPE selects MoT
# or Shared-DiT; inference steps default to that recipe unless overridden.
#
# Use this when SAPIEN and Torch/Streaming-WAM cannot share one environment (the
...
REPO="${REPO:?set REPO=/path/to/Streaming-WAM}"
PY="${PY:?set PY=/path/to/streamingwam-env/bin/python}"
...
CHECKPOINT_FORMAT="${CHECKPOINT_FORMAT:-streamingwam}"
# Only MoT recipes need an ActionDiT init payload; Shared-DiT leaves it unset.
ACTION_INIT="${ACTION_INIT:-}"
RECIPE="${RECIPE:-examples/robotwin/configs/recipes/streamingwam_robotwin_mot_wan22_5b.yaml}"
