#!/bin/bash
# StreamWAM RoboTwin 2.0 rollout driver (SAPIEN client side).
#
...
#
# Runs inside the RoboTwin (SAPIEN) environment. The heavy StreamWAM inference
# runs in separate policy servers (launch_streamwam_robotwin_policy_server.sh);
...
#   bash examples/robotwin/scripts/launch_streamwam_robotwin_rollout.sh
#
...
PY="${PY:-python}"
POLICY_NAME="${POLICY_NAME:-streamwam_client}"
