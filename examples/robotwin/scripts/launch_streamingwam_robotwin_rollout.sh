#!/bin/bash
# Streaming-WAM RoboTwin 2.0 rollout driver (SAPIEN client side).
#
...
#
# Runs inside the RoboTwin (SAPIEN) environment. The heavy Streaming-WAM inference
# runs in separate policy servers (launch_streamingwam_robotwin_policy_server.sh);
...
#   bash examples/robotwin/scripts/launch_streamingwam_robotwin_rollout.sh
#
...
PY="${PY:-python}"
POLICY_NAME="${POLICY_NAME:-streamingwam_client}"
...
SEED="${SEED:-0}"
CKPT_TAG="${CKPT_TAG:-streamingwam}"
